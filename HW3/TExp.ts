...
import { makeLetExp, makeLitExp } from "./L5-ast";
...
export type CompoundTExp = NewProcTExp | TupleTExp | UnionTExp | InterTExp;
export const isCompoundTExp = (x: any): x is CompoundTExp => isProcTExp(x) || isTupleTExp(x) || isUnionTExp(x) || isInterTExp(x);

//Not sure
export type NonTupleTExp = AtomicTExp | NewProcTExp | TVar | UnionTExp | InterTExp;
export const isNonTupleTExp = (x: any): x is NonTupleTExp =>
    isAtomicTExp(x) || isProcTExp(x) || isTypePredTExp(x) || isTVar(x) || isUnionTExp(x) || isInterTExp(x);
...
//Change3
export type TypePredTExp = { tag: "TypePredTExp"; paramTEs: TExp[]; returnTE: TExp; type: TExp};
export const makeTypePredTExp = ( paramTEs: TExp[], returnTE: TExp, type: TExp ): TypePredTExp => ( { tag: "TypePredTExp", paramTEs, returnTE ,type });
export const isTypePredTExp = (x: any): x is TypePredTExp => x.tag === "TypePredTExp";

export type NewProcTExp = ProcTExp | TypePredTExp;

// Uniform access to all components of a ProcTExp
export const procTExpComponents = (pt: NewProcTExp): TExp[] =>
...
const flattenUnion = (tes: TExp[]): TExp[] => //Changed
    (tes.length > 0) ? 
...
// and remove duplicates
// [number, union(number, string)] => [number]
...
export const CExpToTExp = (exp : any): TExp =>
    isNumTExp(exp) ? makeNumTExp() :
    isBoolTExp(exp) ? makeBoolTExp() :
    isStrTExp(exp) ? makeStrTExp() :
    isVoidTExp(exp) ? makeVoidTExp() :
    isAnyTExp(exp) ? makeAnyTExp() :
    isNeverTExp(exp) ? makeNeverTExp() :
    isUnionTExp(exp) ? makeUnionTExp(exp.components.map(CExpToTExp)) :
    isInterTExp(exp) ? makeInterTExp(exp.components.map(CExpToTExp)) :
    makeTVar(exp.var);
...
const removeDuplicatesAndAny = (tes: TExp[]): TExp[] => // Change
    isEmpty(tes) ? tes :
...
export const makeDiffTExp = (te1: TExp, te2: TExp): TExp =>  // Change
    isNeverTExp(te2) || (isUnionTExp(te2) && te2.components.length === 0) ? te1 :
    (isNeverTExp(te1) || isAnyTExp(te2)) ? te1 :
    isAnyTExp(te1) ? isAnyTExp(te2) ? makeNeverTExp() : makeAnyTExp() :
    //(isAnyTExp(te1) && isAnyTExp(te2)) ? makeNeverTExp() :
    //(isAnyTExp(te1) && isNeverTExp(te2)) ? makeAnyTExp() :
    //(isAnyTExp(te1) && isUnionTExp(te2)) ? makeDiffTExp(makeAllTExp(), te2) :
    (isUnionTExp(te1) && isUnionTExp(te2)) ? makeDiffTExp(makeDiffTExp(te1, te2.components[0]), makeUnionTExp(te2.components.slice(1))) ://makeUnionTExp([makeDiffTExp(te1, te2.components[0]), makeDiffTExp(te1, makeUnionTExp(te2.components.slice(1)))]) :
    (isUnionTExp(te1) && isAtomicTExp(te2)) ? makeUnionTExp(te1.components.filter(component => !equals(component, te2))) :
    ((isAtomicTExp(te1) || isTVar(te1)) && (isAtomicTExp(te2) || isTVar(te2)) && equals(te1, te2)) ? makeNeverTExp() :
    //isAnyTExp(te1) ? isAnyTExp(te2) ? makeNeverTExp() : makeAnyTExp() :
    
    te1;

//export const makeAllTExp = (): TExp => makeUnionTExp([makeBoolTExp(), makeAnyTExp(), makeEmptyTupleTExp(), makeNeverTExp(), makeNumTExp(), makeStrTExp(), makeVoidTExp()]);

// SubType comparator
export const isSubType = (te1: TExp, te2: TExp): boolean =>
    ((isAtomicTExp(te1))&& isAnyTExp(te2)) ? true : //Change
    isNeverTExp(te1) ? true : //Change
    (isUnionTExp(te1) && isUnionTExp(te2)) ? isSubset(te1.components, te2.components) : //te1 and te2 are union
    isUnionTExp(te2) ? containsType(te2.components, te1) : //only te2 is union
    ((isProcTExp(te1) || isTypePredTExp(te1)) && (isProcTExp(te2) || isTypePredTExp(te2))) ? checkProcTExps(te1, te2) :
...
export const checkProcTExps = (te1: NewProcTExp, te2: NewProcTExp): boolean => 
...
*/
const parseProcTExp = (texps: Sexp[]): Result<NewProcTExp> => { //Change3
    const pos = texps.indexOf('->');
    const isPred = texps.indexOf('is?');
    const returnValue = parseTExp(texps[isPred + 1]);
    return (pos === -1)  ? makeFailure(`Procedure type expression without -> - ${format(texps)}`) :
           ...
           (pos !== 1) ? makeFailure(`Only one parameter is allowed in type predicate`) :
           (isPred != pos + 1) ? makeFailure(`Wrong type predicate syntax`) :
           //isBoolTExp(returnValue) ?
           bind(parseTupleTExp(texps.slice(0, pos)), (args: TExp[]) =>
            bind(returnValue, (returnTE: TExp) =>
                mapv(parseTExp(texps[isPred + 1]), (type: TExp) =>
                    makeTypePredTExp(args, returnTE, type))));
        //makeFailure(`Return type of type predicate is not boolean (${texps.join(" || ")}) typePred: ${isPred}`);
};
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isTypePredTExp(x) ? bind(unparseTuple(x.paramTEs), (paramTEs: string[]) =>
            mapv(unparseTExp(x.type), (type: string) =>
                [...paramTEs, '->', 'is?', type])) : //Change3
...
const matchTVarsInTProcs = <T1, T2>(te1: TExp, te2: TExp,
        succ: (mapping: Array<Pair<TVar, TVar>>) => T1,
        fail: () => T2): T1 | T2 => //Change2
    ((isProcTExp(te1) || isTypePredTExp(te1)) && (isProcTExp(te2) || isTypePredTExp(te2))) ? matchTVarsInTEs(procTExpComponents(te1), procTExpComponents(te2), succ, fail) :
    fail();
