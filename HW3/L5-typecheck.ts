...
export const typeofExp = (exp: Parsed, tenv: TEnv): Result<TExp> =>
    ...
    (isProcExp(exp) || isTypePredExp(exp))? typeofProc(exp, tenv) :
...
export const typeofAny = (n: AnyExp): AnyTExp => makeAnyTExp();

export const typeofNever = (n: NeverExp): NeverTExp => makeNeverTExp();
...
export const makeInter = (te1: TExp, te2: TExp): TExp =>
    isSubType(te1, te2) ? te1 :
    isSubType(te2, te1) ? te2 : 
    makeInterTExp([te1, te2]);
...
const isTypePredApp = (e: Exp, tenv: TEnv): Result<{ pred: CExp, var: CExp/*, type: TExp */}> => {
    if (isAppExp(e) && isPrimOp(e.rator) && isVarRef(e.rands[0]) && e.rands.length === 1) {
        const pred = e.rator;
        const variable = e.rands[0];
        if(pred.op === 'number?' || pred.op === 'string?' || pred.op === 'boolean?')
            return makeOk({pred: pred, var: variable})
    }
    return makeFailure(`This is not a type predicate application: ${e}}`);
}

export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> =>{
    const predType = isTypePredApp(ifExp.test, tenv);
    //console.log("TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT");
    if(isOk(predType)){
        
        const predTypeApp = predType.value as { pred: CExp, var: CExp}
        const predTE = typeofExp(predTypeApp.pred, tenv);
        const varTE = typeofExp(predTypeApp.var, tenv);
        
        /*const testTE = typeofExp(ifExp.test, tenv);
        const thenTE = typeofExp(ifExp.then, tenv);
        const altTE = typeofExp(ifExp.alt, tenv);*/
            //const constraint1 = bind(testTE, testTE => checkCompatibleType(predTE, varTE, ifExp));
        const constraint1 = bind(varTE, (t1: TExp) =>
            bind(predTE, (t2: TExp) => checkCompatibleType(t1, t2 ,ifExp))
        );
        return bind(constraint1, (_c1: true) =>
            predTE);
    }
...
export const typeofProc = (proc: NewProcExp, tenv: TEnv): Result<TExp> => { //Change3
    if (isProcExp(proc)){
    ...
    } else if (isTypePredExp(proc)){
        const argsTEs = map((vd) => vd.texp, proc.args);
        const extTEnv = makeExtendTEnv(map((vd) => vd.var, proc.args), argsTEs, tenv);
        const constraint1 = bind(typeofExps(proc.body, extTEnv), (body: TExp) => 
                                checkCompatibleType(body, proc.returnTE, proc));
        const returnTE = proc.returnTE;
        return bind(constraint1, _ => makeOk(makeTypePredTExp(argsTEs, returnTE, proc.type)));
    }
    return makeFailure("typeofProc: wrong type of proc");
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> =>  //Change3
    bind(typeofExp(app.rator, tenv), (ratorTE: TExp) => {
        if (! (isProcTExp(ratorTE) || isTypePredTExp(ratorTE))) {
        ...
        if (isTypePredTExp(ratorTE) && isBoolExp(ratorTE.returnTE)) {
            const constraints = zipWithResult((rand, trand) => bind(typeofExp(rand, tenv), (typeOfRand: TExp) => 
                checkCompatibleType(typeOfRand, trand, app)),
                    app.rands, ratorTE.paramTEs); //Change2
            return bind(constraints, _ => makeOk(ratorTE.returnTE));
        }
        else if(isTypePredTExp(ratorTE))
            return makeFailure("Return type of type predicate must be boolean");
...
// Typing rule:
//   (define (var : texp) val)
// VTODO - write the true definition
export const typeofDefine = (exp: DefineExp, tenv: TEnv): Result<VoidTExp> =>
    bind(typeofExp(exp.val, makeExtendTEnv([exp.var.var], [CExpToTExp(exp.val)], tenv)),
