...
export type CompoundExp = AppExp | IfExp | NewProcExp | LetExp | LitExp | LetrecExp | SetExp;
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isTypePredExp(x) || isLitExp(x) || isLetExp(x) || isLetrecExp(x) || isSetExp(x);
export const expComponents = (e: Exp): CExp[] =>
    isIfExp(e) ? [e.test, e.then, e.alt] :
    (isProcExp(e) || isTypePredExp(e)) ? e.body :
...
/*export type AnyExp = {tag: "AnyExp"; val: any; }
export const makeAnyExp = (n: any): AnyExp => ({tag: "AnyExp", val: n});
export const isAnyExp = (x: any): x is AnyExp => x.tag === "AnyExp";

export type NeverExp = {tag: "NeverExp"; val: never; }
export const makeNeverExp = (n: never): NeverExp => ({tag: "NeverExp", val: n});
export const isNeverExp = (x: any): x is NeverExp => x.tag === "NeverExp";
*/
export type AnyExp = AtomicExp;
export type NeverExp = undefined;
//Not sure
...
//Change3
export type TypePredExp = {tag: "TypePredExp"; args: VarDecl[], body: CExp[]; type: TExp; returnTE: TExp }
export const makeTypePredExp = (args: VarDecl[], body: CExp[], type: TExp, returnTE: TExp): TypePredExp =>
    ({tag: "TypePredExp", args: args, body: body, type: type, returnTE: returnTE});
export const isTypePredExp = (x: any): x is TypePredExp => x.tag === "TypePredExp";

export type NewProcExp = ProcExp | TypePredExp;
...
*/
export type PrimOpKeyword = "+" | "-" | "*" | "/" | ">" | "<" | "=" | "not" | "and" | "or" | "eq?" | "string=?" | "union?" | "inter?" |
...
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<NewProcExp> => {
    if (isArray(vars)) {
        const args = mapResult(parseVarDecl, vars);
        const body = mapResult(parseL5CExp, rest[0] === ":" ? rest.slice(2) : rest);
        if (rest[1] === "is?") {
            const returnTE = makeOk(makeFreshTVar());
            const type = parseTExp(rest[2]);
            return bind(args, (args: VarDecl[]) =>
                        bind(body, (body: CExp[]) =>
                            bind(returnTE, (returnTE: TExp) =>
                                bind(type, (type: TExp) =>
                                    makeOk(makeTypePredExp(args, body, type, returnTE))))));
        } else {
...
export const unparse = (e: Parsed): Result<string> =>
    ...
    (isProcExp(e) || isTypePredExp(e))? unparseProcExp(e) :
...
const unparseProcExp = (pe: NewProcExp): Result<string> =>
