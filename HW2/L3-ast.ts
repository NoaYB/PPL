...
const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> =>{
    ...
    */
    ...
    const vars = map(b => b[0], methods); // Takes the name of the methods: [first, second, sum]
    const valsResult = mapResult(parseL3CExp, map(second, methods)); // Make list of parsed lambdas ((lambda () a) for example) to CExp
    const bindingsResult = mapv(valsResult, (vals: CExp[]) => zipWith(makeBinding, vars, vals)); // Make binding from the two lists, for example first binded to (lambda () a)
    /*console.log("methods: "+methods.join(" || "));
    console.log("methods.length: "+methods.length);
    console.log("fields: "+fields);
    console.log("vars:", vars.join(", "));
    console.log("valsResult: "+valsResult);
    console.log("bindingsResult: "+bindingsResult);*/
...
const unparseClassExp = (ce: ClassExp): string => 
    `(class (${ce.fields.map(f => f.var).join(" ")}) (${ce.methods.map(m => `(${m.var.var} ${unparseL3(m.val)})`).join(" ")}))`;
